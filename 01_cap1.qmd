...
A partir de um modelo econômico, podemos especificar o **modelo econométrico** a partir dos dados disponíveis. 
$$crime = \beta_0 + \beta_{1}wage_m + \beta_{2}otherinc + \beta_{3}freqarr + \mu$$
Com a definição dos modelos econométricos, poderíamos levantar a hipótese de que $\beta_2 = 0$. A análise empírica, por definição, requer dados. 
...
O conjunto de dados tem característica tanto de corte transversal quanto de séries temporais.
Muito usado para analisar os efeitos de uma política de governo. Por exemplo, são coletados dados anteriores e posteriores a uma política pública.
